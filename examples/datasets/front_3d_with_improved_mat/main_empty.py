...
import numpy as np
from blenderproc.python.loader.Front3DLoader import load_front3d_no_furniture
...
parser.add_argument(
    "front_3D_texture_path", help="Path to the 3D FRONT texture folder."
)
parser.add_argument(
    "cc_material_path",
    nargs="?",
...
parser.add_argument(
    "output_dir",
    nargs="?",
    default="examples/datasets/front_3d_with_improved_mat/output",
    help="Path to where the data should be saved",
)
parser.add_argument(
    "--lamp_light_strength", type=float, default=5, help="Strength of the lamp light"
)
parser.add_argument(
    "--ceiling_light_strength",
    type=float,
    default=0.5,
    help="Strength of the ceiling light",
)
parser.add_argument(
    "--word_background_strength",
    type=float,
    default=5,
    help="Strength of the world background",
)
...
bproc.renderer.set_light_bounces(
    ...
    transparent_max_bounces=100,
)


# load the front 3D objects
loaded_objects = load_front3d_no_furniture(
...
bproc.world.set_world_background_hdr_img(
    haven_hdri_path, strength=word_background_strength
)


cc_materials = bproc.loader.load_ccmaterials(
    args.cc_material_path, ["Wallpaper", "Bricks", "Wood", "Carpet", "Tile", "Marble"]
)
...
def process_differences(array1, array2):
    ...
    array2[significant_diff] = (
        0  # 255 is commonly used in image contexts to represent white
    )
    ...
    for file in files:
        ...
        if file.endswith(".hdf5"):
            with h5py.File(os.path.join(args.output_dir, file), "r") as f:
                ...
                depth = f['depth'][:]
                depth[depth >= 10] = 10
                np.save(os.path.join(args.output_dir, 'room_depth.npy'), depth)
