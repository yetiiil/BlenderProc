...
bpy.ops.export_scene.obj(filepath=os.path.join(args.output_dir, "room.obj"))
...
proximity_checks = {"min": 2.5, "max": 5, "no_background":False}
...
while tries < 10000 and poses < 1:
    ...
    rotation = np.random.uniform([np.pi / 2, 0, 0], [np.pi / 2, 0, np.pi * 2])
...
with open(os.path.join(args.output_dir, "cam2world_matrix.txt"), "w") as f:
...
for file in files:
    ...
    if file.endswith('.hdf5'):
        with h5py.File(os.path.join(args.output_dir, file), 'r') as f:
            ...
            colors_image.save(os.path.join(args.output_dir, 'room.png'))
            semantic = f['category_id_segmaps'][:]
            np.save(os.path.join(args.output_dir, 'room_semantic.npy'), semantic)
